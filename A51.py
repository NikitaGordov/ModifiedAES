...
def initialize_and_process(plaintextByteArg):
    R1 = R2 = R3 = 0
    ...
    #plainText = str(b'\xff\xfe\xfd')
    #plainTextByte = str.encode(plainText)
    plainTextByte = plaintextByteArg
    ...
    #print("Plaintextbyte is:")
    #print(plainTextByte)
    #print("len plaintextByte", len(plainTextByte))
    #print("\n")

    return finalKey

def encrypt(plaintextByteArg, finalKey):
    plainText = int.from_bytes(plaintextByteArg, byteorder = sys.byteorder)
    ...
    byteEncryptedText =  encryptedText.to_bytes(3, byteorder = sys.byteorder)
    #print("Encrypted plain text value is:")
    #print((byteEncryptedText))
    #print("len ecnrypted", len(byteEncryptedText))
    #print("\n")

    return byteEncryptedText

def decrypt(byteEncryptedText, finalKey):
    #encryptedText = int(byteEncryptedText.encode('hex'), 16)
    #decrypt = encryptedText ^ finalKey
    encryptedText = int.from_bytes(byteEncryptedText, byteorder = sys.byteorder)
    decrypt = encryptedText ^ finalKey
    #byteDecryptedText = decrypt.to_bytes((decrypt.bit_length() // 8) + 1, byteorder = sys.byteorder)
    byteDecryptedText = decrypt.to_bytes(3, byteorder = sys.byteorder)

    #print("Decrypted plain text value is:")
    #print(byteDecryptedText)
    #print("len decrypted", len(byteDecryptedText))
